    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        try:
            ...
            self.log({'negative_loss': printable_neg_loss, 'positive_loss': printable_pos_loss, 'positive_loss_multiplier': multiplier,
                     'loss': loss.detach().cpu().item(), 'change_in_loss': change_in_loss})
...
def unlearn(model_dir, output_dir, forget_dir, retain_dir, trainable_layers = [1,2,3,4,5]):
...
if __name__ == '__main__':
    ...
    parser.add_argument('trainable_layers', type=list, default=[1,2,3,4,5], help='Layers to train')
